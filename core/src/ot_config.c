...
static int ot_config_set(const char *net_name, const u8_t channel,
			 ...
			 const char *masterkey)
{
	...
	// LOG_DBG("Wipe OpenThread credentials");
	// otPlatSettingsWipe(ot_context->instance);
...
int ot_config_update(void)
{
	...
	u8_t channel= 12;			// Channel
	char net_name[NET_NAME_LEN] = "knot";	// Network name
	...
	printk("***** xpanid: %s *****\n", xpanid);
	printk("***** masterkey: %s *****\n", masterkey);
	rc = ot_config_set(net_name, channel, panid, xpanid, masterkey);
